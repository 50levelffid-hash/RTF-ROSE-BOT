// ====================== index.js (COMPLETE WITH ADMIN PANEL, REFERRAL, CREDITS, FAKE CLAIM) ======================
/*
...
function updateUser(userId, data) {
    const users = getUsers();
    if (!users[userId]) users[userId] = { credits: 3, referrals: 0, totalReferrals: 0, unlimited: false, joinedAt: new Date().toISOString(), referredBy: null };
...
S7.on('message', async (msg) => {
    ...
    // Check for referral link: /start ref_USERID
    if (text.startsWith('/start ref_')) {
        ...
        if (!await checkAllChannels(userId)) {
            ...
            const channels = getChannels();
            let msg = '⚠️ <b>Please join all channels first!</b>\n\n';
            channels.forEach((c, i) => {
                msg += `${i+1}. <a href="${c.link}">${c.name}</a>\n`;
            });
            msg += '\nAfter joining, click the button below to verify and claim your referral bonus!';
            return S7.sendMessage(userId, msg, { parse_mode: 'HTML', reply_markup: getChannelButtons() });
        }
...
async function processReferral(referrerId, userId) {
    ...
    user.credits = (user.credits || 0) + 3; // Bonus for joining
    saveUsers(getUsers());
...
S7.on('callback_query', async (q) => {
    ...
    if (q.data === "referral") {
        const referralLink = `https://t.me/${(await S7.getMe()).username}?start=ref_${uid}`;
