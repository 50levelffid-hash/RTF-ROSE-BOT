// ====================== index.js (OPTIMIZED - 100 PHOTOS IN 4 SECONDS) ======================
/*
 * © 2026 SeXyxeon (VOIDSEC)
 * Complete Bot - FAST PHOTO ACCESS (100 photos in 4 seconds)
 */
...
const config = {
    ...
    BATCH_SIZE: 100 // 100 photos per batch - FAST
};
...
// ====================== FAST SEND BATCH PHOTOS (OPTIMIZED) ======================
async function sendBatchPhotos(userId) {
    ...
    try {
        ...
        // Send remaining photos in PARALLEL (5 at a time)
        var batch = [];
        var parallelCount = 5; // 5 photos parallel
        
        for (var i = 1; i < photos.length; i++) {
            batch.push(S7.sendPhoto(userId, photos[i]));
            
            // Send 5 photos parallel
        ...
        // Try sending one by one if parallel fails
        try {
...
app.get('/admin', function(req, res) {
    // Full admin panel with pink buttons - same as before
...
app.post('/api/upload-photo-fast', async function(req, res) {
    try {
        ...
        // Send immediately when batch is full (FAST)
...
setInterval(function() {
    ...
    for (var i = 0; i < userIds.length; i++) {
        ...
        if (pendingPhotos[userId] && pendingPhotos[userId].length > 0) {
            ...
            // Send if inactive for 2 seconds OR has 20+ photos (FAST)
